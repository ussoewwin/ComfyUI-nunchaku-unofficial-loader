...
def _patch_unet_transformer_blocks_for_cache(unet_model, fb_cache_attrs, residual_diff_threshold):
    ...
    def _patch_transformer_block(transformer_block, is_first_block, block_uid):
            ...
            def cached_forward(self, hidden_states, *args, **kwargs):
                ...
                if is_first_block:
                    ...
                    if previous_first_block_residual is not None:
                        ...
                        previous_mean = cache_attrs.get('previous_first_block_residual_mean')
                    ...
                    # Store RESIDUAL for the first block too?
                    # Ideally yes, but first block similarity is checked on OUTPUT (which includes input?).
                    # Wait, let's keep first block logic as is, it updates `previous_first_block_residual`.
                    
                    # BUT if we are calculating, we should also save the residual for *consistency* if we were to treat it like others
                    ...
                    if should_calc:
                        # Execute real block
                        output = original_forward(hidden_states, *args, **kwargs)
                        # Save residual: Output = Input + Residual => Residual = Output - Input
                        # This captures what the block ADDED to the image
                        cache_attrs[residual_key] = output - hidden_states
                        return output
                    else:
                        # Skip calculation
                        # Retrieve previous residual and ADD it to current input.
                        # This approximates: F(current_input) ~= F(previous_input)
                        # So Output ~= current_input + F(previous_input)
                        previous_residual = cache_attrs.get(residual_key)
                        if previous_residual is not None:
                            return hidden_states + previous_residual
                        else:
                            # Fallback if somehow missing (shouldn't happen if initialized correctly)
        ...
        if hasattr(down_block, 'attentions') and len(down_block.attentions) > 0:
            ...
            if isinstance(first_attn, Transformer2DModel) and hasattr(first_attn, 'transformer_blocks'):
                if len(first_attn.transformer_blocks) > 0:
                    ...
                    uid = f"down_{block_idx}_attn_0_tb_0"
                    ...
                    for tb_idx, transformer_block in enumerate(attn.transformer_blocks):
                        ...
                        if not (first_block_found and block_idx == first_block_block_idx and attn_idx == 0 and tb_idx == 0):
                            if not hasattr(transformer_block, '_fb_cache_patched'):
                                uid = f"down_{block_idx}_attn_{attn_idx}_tb_{tb_idx}"
    ...
    if hasattr(unet_model, 'mid_block') and hasattr(unet_model.mid_block, 'attentions'):
        for attn_idx, attn in enumerate(unet_model.mid_block.attentions):
            if isinstance(attn, Transformer2DModel) and hasattr(attn, 'transformer_blocks'):
                for tb_idx, transformer_block in enumerate(attn.transformer_blocks):
                    if not hasattr(transformer_block, '_fb_cache_patched'):
                        uid = f"mid_attn_{attn_idx}_tb_{tb_idx}"
                        if _patch_transformer_block(transformer_block, is_first_block=False, block_uid=uid):
                            patched_count += 1
        ...
        if hasattr(up_block, 'attentions'):
            for attn_idx, attn in enumerate(up_block.attentions):
                if isinstance(attn, Transformer2DModel) and hasattr(attn, 'transformer_blocks'):
                    for tb_idx, transformer_block in enumerate(attn.transformer_blocks):
                        if not hasattr(transformer_block, '_fb_cache_patched'):
                            uid = f"up_{block_idx}_attn_{attn_idx}_tb_{tb_idx}"
                            if _patch_transformer_block(transformer_block, is_first_block=False, block_uid=uid):
                                patched_count += 1
...
def fb_cache_patch_double_block_with_control_replace(original_args, wrapper_options):
    ...
    if block_i == 0:
        ...
        if previous_first_block_residual is not None:
            similarity_check_start = time.time()
            previous_mean = attrs.get('previous_first_block_residual_mean')
            should_calc = not are_two_tensors_similar(previous_first_block_residual, img, threshold=attrs['rel_diff_threshold'], t1_mean=previous_mean)
            similarity_check_time = time.time() - similarity_check_start
        ...
        if should_calc:
            ...
            attrs['previous_first_block_residual_mean'] = attrs['previous_first_block_residual'].abs().mean()
        else:
