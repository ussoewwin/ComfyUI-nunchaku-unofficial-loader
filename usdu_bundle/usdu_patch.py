...
def patch_usdu_linear_and_chess_process():
    ...
    def new_linear_process(self, p, image, rows, cols):
        ...
        def run_batch_mode(effective_batch_size: int):
            vae_encoder = VAEEncode()
            vae_decoder = VAEDecode()
            ...
            for yi in range(rows):
                for xi in range(cols):
                    ...
                    if len(tiles_to_process) >= effective_batch_size or (yi == rows - 1 and xi == cols - 1):
                        batch_count += 1
        ...
        try:
            run_batch_mode(batch_size)
        except RuntimeError as e:
            if "size of tensor" in str(e) or "must match" in str(e) or "QuantizedTensor" in str(e):
                logger.warning("[USDU] Batched run failed with tensor size mismatch (often with quantized FP8/FP4 models). Retrying with batch_size=1: %s", e)
                p.batch_size = 1
                run_batch_mode(1)
            else:
                raise
    ...
    def new_chess_process(self, p, image, rows, cols):
        ...
        def run_chess_batches(effective_batch_size: int):
            vae_encoder = VAEEncode()
            vae_decoder = VAEDecode()
            vae_decoder_tiled = VAEDecodeTiled()
            mask_template, draw_template = self.init_draw(p, image.width, image.height)
            ...
            for color in (True, False):
                ...
                for tx, ty in chess_order_iter(color):
                    ...
                    if len(tiles_to_process) >= effective_batch_size:
                        shared.batch = _process_batch_tiles(p, tiles_to_process, shared.batch, self.calc_rectangle, vae_encoder, vae_decoder, vae_decoder_tiled)
                        tiles_to_process = []
                if tiles_to_process:
                    shared.batch = _process_batch_tiles(p, tiles_to_process, shared.batch, self.calc_rectangle, vae_encoder, vae_decoder, vae_decoder_tiled)
        ...
        try:
            run_chess_batches(batch_size)
        except RuntimeError as e:
            if "size of tensor" in str(e) or "must match" in str(e) or "QuantizedTensor" in str(e):
                logger.warning("[USDU] Chess batched run failed with tensor size mismatch (often with quantized FP8/FP4 models). Retrying with batch_size=1: %s", e)
                p.batch_size = 1
                run_chess_batches(1)
            else:
                raise
